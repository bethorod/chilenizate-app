      ...
      {isExpanded && (
        <CardContent className="pt-0">
          ...
          {historicalPeriodImages[period.id]?.length ? (
            <div className={`mt-6 grid gap-3 ${historicalPeriodImages[period.id].length === 1 ? 'grid-cols-1' : 'grid-cols-2'}`}>
              {historicalPeriodImages[period.id].map((img, i) => (
                <figure key={i} className="overflow-hidden rounded-lg border bg-white/70">
                  <img
                    ...
                    alt={img.title}
                    loading="lazy"
                    className={`w-full object-cover ${historicalPeriodImages[period.id].length === 1 ? 'h-48 sm:h-72' : 'h-32 sm:h-48'}`}
                  />
